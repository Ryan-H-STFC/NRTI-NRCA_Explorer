...
class PeakWindow(QMainWindow):
    ...
    def __init__(self, parent: QWidget | None = ..., flags: Qt.WindowType = ..., index: QModelIndex = None) -> None:
        ...
        try:
            ...
            if parent.maxTableOptionRadio.isChecked():
        ...
        annotationOrder = element.maxAnnotationOrder.items() if parent.maxTableOptionRadio.isChecked(
        ) else element.minAnnotationOrder.items()
        rank = [str([ann[0] for ann in list(annotationOrder) if ann[1][0] == peak[0]][0])]
